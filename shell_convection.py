...
def load_latest_checkpoint(solver, path_repository):
    checkpoint_files = glob.glob(str(path_repository / 'checkpoints/checkpoints_s*.h5'))
    
    if not checkpoint_files:
        raise FileNotFoundError("Aucun fichier checkpoint trouvé dans le répertoire spécifié.")

    latest_checkpoint = max(checkpoint_files, key=lambda x: int(x.split('_s')[-1].split('.h5')[0]))
    print(f"Chargement du dernier checkpoint : {latest_checkpoint}")
